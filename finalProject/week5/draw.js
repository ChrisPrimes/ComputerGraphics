...
var carpetTexture = new THREE.TextureLoader().load("textures/Wool_carpet_pxr128_bmp.png");
var carpetBmpTexture = new THREE.TextureLoader().load("textures/Wool_carpet_pxr128_bmp_bump.png");
carpetTexture.minFilter = THREE.LinearFilter;
carpetTexture.wrapS = THREE.RepeatWrapping;
carpetTexture.wrapT = THREE.RepeatWrapping;
carpetTexture.repeat.set(6, 6);
carpetBmpTexture.minFilter = THREE.LinearFilter;
carpetBmpTexture.wrapS = THREE.RepeatWrapping;
carpetBmpTexture.wrapT = THREE.RepeatWrapping;
carpetBmpTexture.repeat.set(6, 6);
var carpetMaterial = new THREE.MeshPhongMaterial({
  map: carpetTexture,
  bumpMap: carpetBmpTexture
});
/*********************************************/
...
var gridBoxGeom = new THREE.BoxGeometry(100, 0.1, 100);
var gridBoxMesh = new THREE.Mesh(gridBoxGeom, carpetMaterial);
...
document.querySelector("#chooseObjEdit").addEventListener("change", function() {
  ...
  document.querySelector("#colorControls").classList.add('d-none');

  if (arrayElemToEdit.type == "Box" || arrayElemToEdit.type == "Sphere" || arrayElemToEdit.type == "Cylinder" || arrayElemToEdit.type == "Cone" || arrayElemToEdit.type == "Shape") {
    document.querySelector("#colorControls").classList.remove('d-none');
  }
});
...
document.querySelector("#draw-cone").addEventListener("click", function() {
  var radius = parseInt(document.querySelector('#draw-cone-radius').value);
  var height = parseInt(document.querySelector('#draw-cone-height').value);

  var coneGeom = new THREE.ConeGeometry( radius, height, 32 );
  var coneMesh = new THREE.Mesh(coneGeom, sMaterial);
...
document.querySelector("#draw-shape").addEventListener("click", function() {
  ...
  var shapeMesh = new THREE.Mesh(shapeGeom, sMaterial);
...
document.querySelector("#applyColor").addEventListener("click", function() {
  if (!isArrayOfObjectsEmpty()) {
    var obj = getSelectedObjectToEdit();

    var selectedColorHex = document.querySelector("#chooseObjColor").value;

    if (selectedColorHex == "brick") {
      var newMaterial = makeMaterialFromFile("textures/Red_dutch_pxr128.png");
      obj.material = newMaterial;
    } else if (selectedColorHex == "metal") {
      var newMaterial = makeMaterialFromFile("textures/Alloy_diamond_plate_pxr128.png", "textures/Alloy_diamond_plate_pxr128_bump.png");
      obj.material = newMaterial;
    } else if (selectedColorHex == "plad") {
      var newMaterial = makeMaterialFromFile("textures/Checked_cotton_pxr128.png");
      obj.material = newMaterial;
    } else if (selectedColorHex == "marble") {
      var newMaterial = makeMaterialFromFile("textures/Gray_swirl_marble_pxr128.png");
      obj.material = newMaterial;
    } else if (selectedColorHex == "wood") {
      var newMaterial = makeMaterialFromFile("textures/White_maple_pxr128.png");
      obj.material = newMaterial;
    } else if (selectedColorHex == "rock") {
      var newMaterial = makeMaterialFromFile("textures/Rock_in_mortar_pxr128.png", "textures/Rock_in_mortar_pxr128_bump.png");
      obj.material = newMaterial;
    } else {
...
function makeMaterialFromFile(file, bumpFile) {
  var texture = new THREE.TextureLoader().load(file);
  var bumpTexture = new THREE.TextureLoader().load(bumpFile);
